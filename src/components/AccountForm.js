...
class AccountForm extends React.Component {
  ...
  handleAccountNoChange = (event) => {
    // event.preventDefault();
    this.setState({ toaccount_no: event.target.value });
  };
  ...
  handleOnSubmit = (event) => {
    ...
    let { amount, account, toaccount_no } = this.state;

    const { selectedType } = this.props;
    const fieldsToValidate = [{ amount, toaccount_no }];

    const allFieldsEntered = validateFields(fieldsToValidate);
    if (!allFieldsEntered) {
      this.setState({
        errorMsg: {
          [selectedType === "withdraw" || selectedType === "withdraw"
            ? "withdraw_error"
      ...
      if (selectedType === "withdraw" && amount <= total_balance) {
      ...
      } else if (selectedType === "transfer" && amount <= total_balance) {
        this.props.dispatch(
          initiateTransferAmount(account.account_id, amount, toaccount_no)
        );
        this.setState({
          errorMsg: "",
        });
        ...
        this.setState({
          errorMsg: {
            [selectedType === "withdraw" || selectedType === "transfer"
              ? "withdraw_error"
        ...
        <div className="account-form col-md-6 offset-md-3">
          ...
          >
            ...
            {selectedType === "transfer" ? (
              <Form.Group controlId="toaccount_no">
                <Form.Label>Account number: </Form.Label>
                <Form.Control
                  type="number"
                  //   name="toaccount_no"
                  placeholder="Enter your account number"
                  value={this.state.toaccount_no}
